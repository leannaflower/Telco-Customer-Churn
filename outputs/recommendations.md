...
## Finding 1
- **What the data shows:** Contract type is the single strongest churn driver. Month-to-month customers churn at 42.7%, vs. 11.3% for one-year and just 2.8% for two-year contracts, which is roughly **15x** the churn rate of two-year customers.
...
- **Recommendation:** Target month-to-month customers with a discounted annual-contract upgrade offer, timed around the highest-risk tenure window.
...
- **Supporting evidence:** `tenure` ranks #5 by mean |SHAP value|, and the engineered `charge_ratio` (total spend relative to monthly rate, which is naturally low for new accounts) ranks #2 overall. This is the single strongest non-contract signal in the model.
...
- **Estimated impact:** 1,481 customers (21% of the base) are in month 0-6, accounting for 784 of the dataset's ~1,869 total churned customers. This is disproportionate over 40% of all churn from a fifth of the customer base.
